...
- Layering is `routes` → `service` → `repository`; the assistant's tools (`agent/TaskTools`) must go through `TaskService`, never the repository directly. Service beans are declared in a Koin module (`config/KoinConfig.kt`) and resolved only at the composition root (`Application.module`, tests override via its `koinModules` parameter) — layers receive dependencies as plain parameters; env config stays as `module()` parameters, not beans.
