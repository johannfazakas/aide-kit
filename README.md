...
- The task store is chosen by the active startup profile at the composition root (see [Configuration](#configuration)): `local` (default) uses an ephemeral in-memory store, `live` uses a git clone of an Obsidian vault that is the live source of truth. The vault backend supports list, get, create, and update; delete returns `501` (deferred). An update is a single synchronized transaction — one pull per mutation — that merges a per-field patch (only the fields the caller named) onto freshly pulled state, then rewrites surgically: it changes only the lines backing changed fields and preserves everything else byte-for-byte, including line terminators (CRLF is kept), free-text notes, and fields the model doesn't know. Dataview inline fields written on the checkbox line itself (e.g. `- [ ] Pay rent [due:: 2026-09-01]`) are read; when edited they migrate to their own follow-up line (an untouched inline field stays in place). The known-topics list is vault-owned (`organization/Topics.md`) under the `live` profile and a seeded list under `local`.
- Concurrent edits are last-writer-wins on the fields a mutation names. Because each update re-pulls and merges only its own fields onto fresh state, a concurrent edit to a *different* field is never reverted, and on the `live` vault a git rebase/push conflict surfaces as `409` with nothing written (git history preserves any overwritten value).
...
A task is `{"id", "title", "dueDate"?, "topic"?, "done"}` — `topic` groups it, `done` is its completion state (the former `category`/`completed` names are rejected). `title` must be a single line (line breaks and control characters are rejected).

Errors are JSON `{"message": "..."}`: `400` invalid input (including an unknown topic or a multi-line title), `404` unknown id, `409` vault conflict, `501` operation unsupported by the storage backend, `502` LLM gateway failure.
