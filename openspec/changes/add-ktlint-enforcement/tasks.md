...
- [ ] 2.1 Run `./gradlew ktlintFormat`, review the diff, and fix any violations ktlint cannot auto-correct
- [ ] 2.2 Verify `./gradlew build` passes (lint + tests) and commit the reformat as a dedicated commit separate from the tooling changes

## 3. CI workflow

- [ ] 3.1 Create `.github/workflows/build.yml`: triggers on push to `main` and pull requests; checkout → `actions/setup-java` (Temurin 21) → `gradle/actions/setup-gradle` → `./gradlew build`; no secrets
...
- [ ] 4.2 Register the script as a `PostToolUse` hook matching `Edit|Write` in the project's `.claude/settings.json`
- [ ] 4.3 Test the hook manually: pipe sample hook JSON for a compliant file, a violating file, and a non-Kotlin file; verify exit codes 0/2/0

## 5. Documentation

- [ ] 5.1 Update `CLAUDE.md`: note ktlint enforcement, the `ktlintFormat`/`ktlintCheck` commands, and that the agent should run `ktlintFormat` rather than hand-fixing style errors
- [ ] 5.2 Update `README.md` with the lint commands and a note about the CI workflow
